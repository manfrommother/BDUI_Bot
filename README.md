...
- ✅ **Пропуск дат**: возможность вручную указать даты, когда не нужно проводить дейли (`/skip`, `/unskip`, `/skiplist`)
- ✅ **Права администратора**: команды управления (`/add`, `/remove`, `/setchat`, `/addall`, `/skip`) доступны только администратору
...
- `/unskip ДД.ММ.ГГ` — убрать дату из списка пропусков.
